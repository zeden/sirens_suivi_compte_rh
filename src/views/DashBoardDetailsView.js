...
export default class DashBoardDetailsView extends React.Component {
  ...
  state = {
    ...
    currentDataSourceIndicateur:
      FILTRES_INDICATEURS[lastIndicateurSelectIndex].Agents,
    lastIndicateurSelectIndex:
      FILTRES_INDICATEURS[lastIndicateurSelectIndex].value,
    currentDataSourceRecherche:
      FILTRES_RECHERCHE[lastRechercheSelectIndex].Agents,
  ...
  exportSelection = (selectedRows) => {
    ...
    link.setAttribute(
      "download",
      "liste_agents_" + new Date().toISOString() + ".csv"
    );
  ...
  onChangeIndicateurSelect = (value, option) => {
    let Filtre = FILTRES_INDICATEURS.filter(
      (Filtre) => Filtre.value === value
    )[0];
  ...
  onChangeRechercheSelect = (value, option) => {
    let Filtre = FILTRES_RECHERCHE.filter(
      (Filtre) => Filtre.value === value
    )[0];
    let data = Filtre.Agents;
            ...
            >
              <Select
                ...
                filterOption={(input, option) =>
                  option.searchin.toLowerCase().indexOf(input.toLowerCase()) >=
                  0
                }
              >
                {FILTRES_INDICATEURS.map((item, index) => (
                  ...
                  >
                    ...
                    >
                      <span>
                        <Badge
                          status={this.WARNINGS[!(item.Agents.length === 0)]}
                        />
                        {index + 1} - {item.name}
                      </span>
                ...
                >
                  ...
                  >
                    <Button
                      type="primary"
                      onClick={() =>
                        this.exportSelection(this.state.selectedRows)
                      }
                      shape="circle"
